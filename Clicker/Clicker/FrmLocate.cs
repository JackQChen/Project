...
namespace Clicker
{
    public partial class FrmLocate : Form
    {
        ...
        private void FrmLocate_Paint(object sender, PaintEventArgs e)
        {
            for (int i = 0; i < 6; i++)
            {
                e.Graphics.FillRectangle(Brushes.Blue, pStart.X, pStart.Y + (pEnd.Y - pStart.Y) * i / 5 - 3, pEnd.X - pStart.X, 5);
                e.Graphics.FillRectangle(Brushes.Blue, pStart.X + (pEnd.X - pStart.X) * i / 5 - 3, pStart.Y, 5, pEnd.Y - pStart.Y);
            }
